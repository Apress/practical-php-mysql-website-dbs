...
<html lang=en>
<head>
<title>Edit a member's account address</title>
<meta charset=utf-8>
...
#submit { float:left; margin-bottom:20px;}
</style>
</head>
<body>
<div id="container">
...
<div id="content"><!--Start of page content for editing an address-->
<h2>Edit Your Account</h2>
<?php 
// After clicking the Edit link in the register_found_record.php page. This editing interface appears 
// Look for a the user's id, either through GET or POST
if ( (isset($_GET['id'])) && (is_numeric($_GET['id'])) ) { // From view_users.php
	$id = $_GET['id'];
} elseif ( (isset($_POST['id'])) && (is_numeric($_POST['id'])) ) { 
	$id = $_POST['id'];
} else { // The id was not found, stop the script
...
$result = @mysqli_query ($dbcon, $q);
if (mysqli_num_rows($result) == 1) { // The user's id was valid, display the form.
	// Fetch the user's information
	$row = mysqli_fetch_array ($result, MYSQLI_NUM);
	// Create the form
echo '<form action="safe_edit_address_account.php" method="post">
...
} else { // If the user could not be validated
	echo '<p class="error">This page has been accessed in error.</p>';
}
mysqli_close($dbcon);
